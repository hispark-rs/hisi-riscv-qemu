...
> 设计见 [`docs/explanation/design-rationale.md`](docs/explanation/design-rationale.md),使用见 [`docs/how-to/`](docs/how-to/),
> 内存映射见 [`docs/reference/memory-map.md`](docs/reference/memory-map.md),变更见 [`CHANGELOG.md`](CHANGELOG.md)。
...
- **v0.3.0** — **C SDK 外设样例测试台**(`csdk-test.sh` + `tests/csdk/`,5/5 进 CI)、**NV / 分区表 flash overlay**、
  **看门狗 ROM API 仿真**(喂狗→重装、超时→复位)、LSADC/DMA/本地中断投递的模型修复、`docs/reference/rom-stubs.md` +
  `docs/how-to/` 操作指南。
...
- **HiSilicon riscv31 `xlinx` 自定义指令集已实现**(`l.li`/`*shf`/`b*i`/`muliadd`/`jal16`/`j16`/
  ...
  详见 [`docs/reference/xlinx-isa.md`](docs/reference/xlinx-isa.md)。
...
- **原先的两条「硬边界」均已解除**:
  1. **掩膜 ROM** → 不再是阻塞:`ws63_rom_call` 在宿主 C 里**仿真**用到的 ROM 函数,未识别地址优雅降级返回 0。
     详见 [`docs/reference/rom-stubs.md`](docs/reference/rom-stubs.md)。
...
- **GDB** ✅ — QEMU 内置 gdbstub 对本机器开箱即用;`run.sh` 透传 `-s -S` 调试,文档见 [`docs/how-to/debug-and-trace.md`](docs/how-to/debug-and-trace.md)。
...
system reset、IO_CONFIG pinmux 路由亦已建模。完整矩阵见 [`docs/reference/peripheral-matrix.md`](docs/reference/peripheral-matrix.md)。

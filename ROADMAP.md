...
## 里程碑：运行厂商 C SDK 固件（xlinx 自定义 ISA）

> 2026-06 新增。原路线图聚焦验证 ws63-rs；此里程碑把仿真器扩展为**也能运行 fbb_ws63 C SDK
> 厂商 gcc 编译的固件**，实现「跑 rs + c 两侧 SDK 多角度对齐」的目标。

- **实现了 HiSilicon riscv31 `xlinx` 自定义指令集**（`l.li`/`*shf`/`b*i`/`muliadd`/`jal16`/`j16`/
  `ldmia`/`stmia`/`push`/`pop`/`popret`/`uxtb`/`uxth`/ 压缩 `lbu`/`lhu`/`sb`/`sh`）——厂商固件通篇使用，
  缺之则 stock QEMU 在第二条指令即陷阱。编码经厂商汇编器逐字段实测，13/13 自检通过。详见
  [`docs/xlinx-isa.md`](docs/xlinx-isa.md)。
- **C SDK `flashboot` 已跑出 UART 输出**（自包含、零 ROM 依赖）：时钟 bring-up → flash init。
- **C SDK 主应用 `ws63-liteos-app`** 正确执行数百万条指令（relocation/dyn_mem_cfg/patch_init/驱动初始化）。
- **两条硬边界**（非 ISA 问题，已表征）：
  1. **掩膜 ROM**：主应用调用 53 个固化在硅片 ROM 的函数，SDK 不提供 ROM 二进制 → 需 ROM dump 或桩。
  2. **SFC Flash 控制器**：bootloader 下一阻塞；建模它 + flash 后端可让 flashboot 加载 app。
- 新增 device：`ws63-tcxo`（时钟计数器）、PPB RAM 区（`0xE0000000` FlashPatch + SCS）；启动参数 a0 握手。

→ 后续：阶段 3 把 **SFC** 纳入（让 flashboot 加载/校验 app），并按需为掩膜 ROM 提供桩函数。

...
| **Flash XIP 窗口** | `0x200000`(8MB) | RAM 背靠,**默认空**;`run.sh NV=1` 用 `-device loader` 回填分区表(`partition_params.bin`@`0x200000`,表在 `0x200380`,magic `0x4b87a54b`)+ NV(见 [csdk 测试 fixtures](https://github.com/hispark-rs/hisi-riscv-qemu/blob/master/tests/csdk/flash/manifest.txt)) |

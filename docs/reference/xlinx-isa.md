...
[`src/target/riscv/insn_trans/trans_xlinx.c.inc`](https://github.com/hispark-rs/hisi-riscv-qemu/blob/master/src/target/riscv/insn_trans/trans_xlinx.c.inc)。
...
- `target/riscv/translate.c`（见 `patches/<QEMU_TAG>/0001-target-riscv-*.patch`）最小 hook：
...
- **必须关闭 Zcb/Zcmp/Zcmt**（由 target/riscv 的 `ws63` 命名 CPU `rv32_ws63_cpu_init` 设定——它是机器默认 CPU 型号）：它们与 xlinx 的 funct3=100 push/pop/uxt
...
- **真实 C SDK `flashboot` 已在 QEMU 上跑出 UART 输出**（厂商 gcc 编译、自包含、零 ROM 依赖）：
  ...
  即 xlinx ISA + 内存映射 + 启动参数(a0) + TCXO 时钟 + UART 全链路打通。SFC Flash 控制器**现已建模**
  （命令接口 RDID/RDSR + flash XIP 窗口，见 [ROM 桩目录](rom-stubs.md) §B / [内存映射](memory-map.md)）；
  完整 flash init 仍需真实分区表/flash 内容（`run.sh NV=1` 回填），否则 bootloader 优雅报错继续、不挂死。rs SDK 各例无回归。
...
并返回 `ra`。详见 [ROM 桩目录](rom-stubs.md)。
...
- Rust 工具链是否需要实现 xlinx → [Rust 工具链是否需要 xlinx](../explanation/rust-toolchain-xlinx.md)
- ROM 拦截机制与清单 → [ROM 桩目录](rom-stubs.md)
- 更广的 ROM 数据墙边界 → [已知边界与非目标](../explanation/limitations.md)

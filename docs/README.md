...
- [运行测试](how-to/run-tests.md) —— 寄存器级 qtest、真实固件冒烟、C SDK 样例。
...
- [验证覆盖范围](reference/verification.md) —— CI 门禁、冒烟固件、C SDK 样例、覆盖边界。
...
真值来源：内存布局 = `ws63-rs/ws63-rt/{memory.x,layout.ld}`；外设基址/寄存器 = `ws63-rs/ws63-pac/ws63-svd/WS63.svd`；

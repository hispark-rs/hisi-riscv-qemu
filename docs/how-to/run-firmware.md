...
本指南讲怎么在 ws63-qemu 上运行一个固件 ELF——包括基本运行、`run.sh` 各开关、以及运行 fbb_ws63 C SDK 固件
（含用 `NV=1` 回填分区表/NV）。`run.sh` 全部选项的清单见 [运行选项参考](../reference/run-options.md)。
...
仿真器除了跑 ws63-rs（Rust，标准 rv32imfc），也能跑 fbb_ws63 **C SDK** 厂商 gcc 编译的固件（依赖 xlinx 自定义
ISA，已实现）。这类固件可由 [`hispark-rs/fbb_ws63-qemu`](https://github.com/hispark-rs/fbb_ws63-qemu) 构建——
...
| `firmware ELF not found` | `run.sh` 第一个参数给对 ELF 路径，或先构建固件 |

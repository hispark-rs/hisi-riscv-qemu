...
| CPU | 单 hart，`riscv_hart_array`，默认核 `sifive-e34`（rv32imafc ⊇ rv32imfc） |
...
1. **CPU 多了 `A` 扩展**（用 sifive-e34）。可加一个自定义 `ws63` 命名 CPU（`riscv_cpu_set_misa_ext(RVI|RVM|RVF|RVC)`，去掉 A）以完全保真——需补丁 `target/riscv/cpu.c`。
2. **无中断控制器**。要驱动中断式驱动 / 连接性，需实现 `ws63-intc`（SYS_CTL1 窗口 + 局部中断线接 CPU）。
3. **SYS_CTL0 / CLDO_CRG 仅吸收**。要让调用 `init_clocks()` 的固件完整跑通时钟初始化，需把 TCXO 检测、
   PLL 锁定位建模为返回「已锁定」。
4. **外设多为吸收**。GPIO/Timer/SPI/I2C 等可按 SVD 增量建模（带状态、可追踪引脚电平）。

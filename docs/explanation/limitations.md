...
### 6. 时钟树按标称建模，频率相关的「错时钟/错波特」症状无法复现

QEMU 把时钟树建模在**标称（post-clock_init）值**（CPU 240 MHz、UART 160 MHz PLL、定时器 24 MHz TCXO、PLL
报告已锁）。**这是 QEMU 的固有边界，不是可建模能解的缺陷**，原因有三：

1. **PLL 由掩膜 boot ROM 锁定**——HAL `clock_init` 注释明确「CPU PLL（240 MHz）由 boot ROM 在 bootloader
   之前配置」。`-kernel` 启动**跳过 boot ROM**，直接从 app 入口执行，此时已是「boot ROM 之后」状态，**PLL 本就
   应报已锁**（恒报已锁是正确的；改成启动未锁反而错——`probe_clocks()` 会读到错值）。
2. **TCG 不建模 CPU 频率**——指令以宿主速度执行（`-icount` 仅 IPC=1 近似）。所以依赖 CPU 真实频率的**忙等延时**
   （如 `delay_ms` 在低频启动时钟下变慢）在 QEMU 里**不会变慢**——这正是「非周期精确」（见 [§1](#1-默认非周期精确虚拟时间自由运行)）的同一根因。
3. **chardev 不限速**——UART 字节无论分频器配成什么都照样发出，所以「错分频器→波特率乱码」在 QEMU 里**也复现不了**，
   只能在写分频器时**标注**有效波特率（`-d guest_errors`，见 [运行选项](../reference/run-options.md)）。

**能做与不能做**：时钟**源选择 / 门控**已建模为状态并影响定时器/外设的模型频率（清定时器门会冻结定时器，已实测）；
但「跳过 clock_init 就在 QEMU 里也算错时序」这一行为**复现不了**——固件在 QEMU 里跑得「时序正确」不代表在硅片上正确。
真周期级请用 gem5 等。对应 issue #3（已说明并 close as not planned）；写分频器/CLK_SEL 时的诊断标注是现实上限。

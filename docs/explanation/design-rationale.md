...
- 新增单文件 `hw/riscv/ws63.c`（机器模型 + 外设设备）+ `insn_trans/trans_xlinx.c.inc`（xlinx 解码器）+
  ...
  走**按版本分目录的 patch-series**（见 [patch-series 参考](https://github.com/hispark-rs/hisi-riscv-qemu/blob/master/patches/README.md)）。
...
- patch-series 结构 → [patch-series 参考](https://github.com/hispark-rs/hisi-riscv-qemu/blob/master/patches/README.md)
